namespace DotAllCombo.Heroes
{
    ...
    internal class UrsaController : HeroController
    {
        ...
        public override void OnInject()
        {
            ...
            DebugExtensions.Chat.PrintSuccess("One Hit to Kill");
        ...
        public override void OnUpdateEvent(EventArgs args)
        {
            ...
            var qslow = e.Modifiers.Any(y => y.Name == "modifier_ursa_earthshock");
            var Wmod = e.Modifiers.Any(y => y.Name == "modifier_ursa_overpower");
            ...
            if (Active && me.Distance2D(e) <= 1400 && e != null && e.IsAlive && !ModifInv)
            {
                    ...
                    )
                {
                    ...
                    R != null && R.CanBeCasted() && me.Distance2D(e) <= me.AttackRange + 30
                    && qslow
                    && menu.Item("Skills").GetValue<AbilityToggler>().IsEnabled(W.Name)
                    && Utils.SleepCheck("W")
                    )
                ...
                {
                    if (mail != null && mail.CanBeCasted() && HasStun(e) && !e.IsStunned() &&
                        menu.Item("Items").GetValue<AbilityToggler>().IsEnabled(mail.Name) && Utils.SleepCheck("mail"))
                    {
                        mail.UseAbility();
                        Utils.Sleep(100, "mail");
                    }
                    if (bkb != null && bkb.CanBeCasted() && HasStun(e) && !e.IsStunned() &&
                        menu.Item("Items").GetValue<AbilityToggler>().IsEnabled(bkb.Name) && Utils.SleepCheck("bkb"))
                    {
                        bkb.UseAbility();
                        Utils.Sleep(100, "bkb");
