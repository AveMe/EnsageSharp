namespace DotAllCombo.Heroes
{
    ...
    class ArcWardenController : HeroController
    {
		#region Variables

		private static Hero _mainHero, me;
		private static bool _loaded;
		private static Vector3 _pushLaneTop = new Vector3(-5895, 5402, 600);
		private static Vector3 _pushLaneBot = new Vector3(5827, -5229, 600);
		private static Vector3 _pushLaneMid = new Vector3(1, 1, 600);

		private static readonly Dictionary<Vector3, string> LaneDictionary = new Dictionary<Vector3, string>()
		{
			{new Vector3(-5895, 5402, 600), "top"},
			{new Vector3(-6600, -3000, 600), "top"},
			{new Vector3(2700, 5600, 600), "top"},


			{new Vector3(5827, -5229, 600), "bot"},
			{new Vector3(-3200, -6200, 600), "bot"},
			{new Vector3(6200, 2200, 600), "bot"},


			{new Vector3(-600, -300, 600), "middle"},
			{new Vector3(3600, 3200, 600), "middle"},
			{new Vector3(-4400, -3900, 600), "middle"}
		...
		override public void OnUpdateEvent(EventArgs args)
		{
			_mainHero = Variables.me;
				...
				if (nearestTower != null)
				{
						...
						Utils.SleepCheck("shield" + handle))
					{
						...
						if (spell != null && spell.CanBeCasted())
						{
							...
							Utils.Sleep(1000, "shield" + handle);
						}
		...
		private static void DoCombo2(Hero me, Hero target)
		{
			...
			var illusions = ObjectManager.GetEntities<Hero>().Where(x => x.IsAlive && x.IsControllable && x.Team == me.Team && x.IsIllusion && x.Modifiers.Any(y => y.Name != "modifier_kill")).ToList();
		...
		private static void DoCombo(Hero me, Hero target)
		{
			...
			if (Menu.Item("order").GetValue<StringList>().SelectedIndex == (int)Orders.Caster && !Menu.Item("hotkeyClone").GetValue<KeyBind>().Active)
			{
				foreach (var hero in Objects.Tempest.GetCloneList(me))
				{
					...
					}
					{
						if (!Utils.SleepCheck("clone_attacking" + hero.Handle)) continue;
						hero.Attack(target);
						Utils.Sleep(380, "clone_attacking" + hero.Handle);
					}
				}
			}
			var illusions = ObjectManager.GetEntities<Hero>().Where(x => x.IsAlive && x.IsControllable && x.Team == me.Team && x.IsIllusion && x.Modifiers.Any(y => y.Name != "modifier_kill")).ToList();
			...
			{
				illusion.Attack(target);
				Utils.Sleep(400, "clone_attacking" + illusion.Handle);
			}
			var necr = Objects.Necronomicon.GetNecronomicons(me);
			foreach (var necronomicon in necr.TakeWhile(illusion => Utils.SleepCheck("clone_attacking" + illusion.Handle) && illusion.Distance2D(target) <= 1500 && !Equals(illusion, me)))
			{
				...
				{
					spell.UseAbility(target);
					Utils.Sleep(500, spell.Name + "clone_attacking" + necronomicon.Handle);
				}
				Utils.Sleep(600, "clone_attacking" + necronomicon.Handle);
			}
			if (!me.IsAlive) return;
		...
		private static void SpellsUsage(Hero me, Hero target, float distance, bool daggerIsReady)
		{
			...
			var enemy = ObjectManager.GetEntities<Hero>().Where(x => x.Team != me.Team && x.IsAlive && x.IsVisible && !x.IsIllusion).ToList();
			if (w != null && w.CanBeCasted())
			{
				foreach (var v in enemy)
				{
					if(Utils.SleepCheck(w.Name) && me.Modifiers.All(x => x.Name != "modifier_arc_warden_magnetic_field") && v.NetworkActivity != NetworkActivity.Attack && v.Distance2D(me) <= 750 && !daggerIsReady)
					{
						w.UseAbility(Prediction.InFront(me, 150));
