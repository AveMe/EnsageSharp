namespace DotAllCombo.Heroes
{
    ...
    internal class BrewmasterController : HeroController
    {
        ...
        private static Item blink, bkb, orchid, necronomicon, urn, medal, shiva, manta;
        ...
        public override void OnInject()
        {
            ...
            Console.WriteLine(">Brewmaster by NeverMore loaded!");
            Menu.AddItem(
        ...
        public override void OnUpdateEvent(EventArgs args)
        {
                ...
                if (me.CanCast())
                {
                    ...
                    if (me.Distance2D(mousepos) <= 1200)
                    {
                        ...
                        if ((W != null && Menu.Item("Skills: ").GetValue<AbilityToggler>().IsEnabled(W.Name)) &&
                            W.CanBeCasted() &&
                            ((target.Position.Distance2D(me.Position) < 850) &&
                             (target.Position.Distance2D(me.Position) > 300)) && R.CanBeCasted() &&
                            Utils.SleepCheck("W"))
                        {
                            W.UseAbility(target);
                            Utils.Sleep(150, "W");
                        ...
                        if ((W != null && Menu.Item("Skills: ").GetValue<AbilityToggler>().IsEnabled(W.Name)) &&
                            W.CanBeCasted() && target.Position.Distance2D(me.Position) < 850 &&
                            (!R.CanBeCasted() || (target.Health < (target.MaximumHealth*0.50)) ||
                             !(Menu.Item("Skills: ").GetValue<AbilityToggler>().IsEnabled(R.Name))) && !CheckDrunken &&
                            (target.Health > (target.MaximumHealth*0.07)) && Utils.SleepCheck("W"))
                        {
                            W.UseAbility(target);
                            Utils.Sleep(150, "W");
                        }
                        ...
                        if ((blink != null && Menu.Item("Items: ").GetValue<AbilityToggler>().IsEnabled(blink.Name)) &&
                            blink.CanBeCasted() && target.Position.Distance2D(me.Position) <= 1150 &&
                            target.Position.Distance2D(me.Position) > 300 && Utils.SleepCheck("blink"))
                        {
                        ...
                        if ((orchid != null && Menu.Item("Items: ").GetValue<AbilityToggler>().IsEnabled(orchid.Name)) &&
                            orchid.CanBeCasted() && (target.Position.Distance2D(me.Position) < 300) &&
                        ...
                        if (necronomicon != null && necronomicon.CanBeCasted() &&
                            (target.Health > (target.MaximumHealth*0.20)) &&
                            (target.Position.Distance2D(me.Position) < 400) && Utils.SleepCheck("necronomicon"))
                        {
                        ...
                        if ((Q != null && Menu.Item("Skills: ").GetValue<AbilityToggler>().IsEnabled(Q.Name)) &&
                            Q.CanBeCasted() && (target.Position.Distance2D(me.Position) < 300) && Utils.SleepCheck("Q"))
                        {
                            Q.UseAbility();
                            Utils.Sleep(150, "Q");
                        }
                        ...
                        if ((medal != null && Menu.Item("Items: ").GetValue<AbilityToggler>().IsEnabled(medal.Name)) &&
                            medal.CanBeCasted() && (target.Position.Distance2D(me.Position) < 300) &&
                        ...
                        if ((shiva != null && Menu.Item("Items: ").GetValue<AbilityToggler>().IsEnabled(shiva.Name)) &&
                            shiva.CanBeCasted() && (target.Position.Distance2D(me.Position) <= 800) &&
                        ...
                        //manta
                        if ((manta != null && Menu.Item("Items: ").GetValue<AbilityToggler>().IsEnabled(manta.Name)) &&
                            manta.CanBeCasted() && (target.Position.Distance2D(me.Position) <= 450) &&
                        ...
                        if ((R != null && Menu.Item("Skills: ").GetValue<AbilityToggler>().IsEnabled(R.Name)) &&
                            R.CanBeCasted() && (target.Position.Distance2D(me.Position) < 500) &&
                            (target.Health > (target.MaximumHealth*0.35)) && !Q.CanBeCasted() && !orchid.CanBeCasted() &&
                        ...
                        //Moving+Attaking
                        if (
                            (!me.CanAttack() || me.Distance2D(target) >= 0) &&
                            me.NetworkActivity != NetworkActivity.Attack && me.Distance2D(target) <= 455 &&
                            ...
                            )
                        {
                            ...
                            me.Distance2D(target) <= 150 && (!R.CanBeCasted() || R == null) &&
            ...
            if (Game.IsKeyDown(Menu.Item("Combo Key").GetValue<KeyBind>().Key))
            {
                if (target.IsAlive && !target.IsInvul() &&
                    (Game.MousePosition.Distance2D(target) <= 1000 || target.Distance2D(me) <= 600))
                {
                    var CheckDrunken = target.Modifiers.Any(y => y.Name == "modifier_brewmaster_drunken_haze");
                            ...
                            ObjectManager.GetEntities<Unit>()
                                .Where(
                                    unit =>
                                        ...
                                        unit.IsControllable)
                                .ToList();
                        if (archer != null && target.Position.Distance2D(v.Position) <= 650 &&
                    ...
                    foreach (var v in primalearth)
                    {
                        ...
                        if (target.Position.Distance2D(v.Position) < 300 && v.Spellbook.SpellR.CanBeCasted() &&
                            !CheckClap &&
                            Utils.SleepCheck(v.Handle.ToString()))
                        {
                        ...
                        if ((target1.Health > (target1.MaximumHealth*0.85)) || ((target1.IsChanneling())))
                        {
                            foreach (var v in primalstorm)
                            {
                                ...
                                if (v.Spellbook.SpellW.CanBeCasted() &&
                                    ...
                                    ((target1.Position != target.Position) || (target1.IsChanneling())) &&
