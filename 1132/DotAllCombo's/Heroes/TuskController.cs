namespace DotAllCombo.Heroes
{
    ...
    internal class TuskController : HeroController
    {
        ...
        {
            AssemblyExtensions.InitAssembly("NeverMore", "0.01");

            DebugExtensions.Chat.PrintSuccess("One-Punch Man! xD");
        ...
        public override void OnUpdateEvent(EventArgs args)
        {
            ...
            if (Active && target.IsAlive && !target.IsInvul() && !target.IsIllusion)
            {
                ...
                var linkens = target.Modifiers.Any(x => x.Name == "modifier_item_spheretarget") ||
                              target.Inventory.Items.Any(x => x.Name == "item_sphere");
                var ModifW = me.Modifiers.Any(x => x.Name == "modifier_tusk_snowball_movement");
                var ModifInv =
                    me.IsInvisible();
                var medallModiff =
                    target.Modifiers.Any(x => x.Name == "modifier_item_medallion_of_courage_armor_reduction") ||
                    target.Modifiers.Any(x => x.Name == "modifier_item_solar_crest_armor_reduction");
                ...
                if ( //R Skill
                    ...
                    R != null
                    && (medallModiff
                        || target.IsMagicImmune() || medall == null)
                    && R.CanBeCasted()
                    && me.CanCast()
                    && !linkens
                ...
                if (SiglControl)
                {
                    foreach (var z in Sigl)
                    {
                        if (target.Position.Distance2D(z.Position) < 1550 &&
                            Utils.SleepCheck(z.Handle.ToString()))
                        {
                            z.Follow(target);
                            Utils.Sleep(700, z.Handle.ToString());
                        }
                    }
                }
            }
