namespace DotAllCombo.Heroes
{
    ...
    internal class ZuusController : HeroController
    {
            ...
            if (menu.Item("autoUlt").GetValue<bool>() && me.IsAlive)
            {
                foreach (var e in enemies)
                {
                    ...
                    if (R != null && e != null && R.CanBeCasted()
                        ...
                        &&
                        !e.Modifiers.Any(y => y.Name == "modifier_ember_spirit_sleight_of_fist_caster_invulnerability")
                        &&
                        !e.Modifiers.Any(y => y.Name == "modifier_ember_spirit_sleight_of_fist_caster_invulnerability")
                        ...
                        && enemies.Count(x => x.Health <= (damage - e.HealthRegeneration-20)) >= (menu.Item("Heel").GetValue<Slider>().Value)
                        && Utils.SleepCheck(e.Handle.ToString()))
                    {
                        R.UseAbility();
                        Utils.Sleep(150, e.Handle.ToString());
